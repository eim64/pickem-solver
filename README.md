...
$$
So after computing $\alpha_i$, $\beta_i$, $\gamma$ (one pass over N)they can be reused to compute the EV almost instantly. Furthermore, we can iterate over A, B and greedily choose the best 6 3-1, 3-2 teams.
...
The generated `precomputed.hpp` contains:

- excluded-set masks `E` (`EXCL_SET_MASKS`);
- 6 A-role masks per excluded set (`ROLE_A_PICK`);
- excluded/free team-position maps (`E_TEAM_POS`, `E_FREE_POS`);
- local 12-bit middle masks and inverse index (`MID_MASKS`, `MID_INDEX`);
- extreme-score lookup `(excluded set, role, extr_roles) -> k` (`EXTR_SCORE_FROM_ROLES`).
